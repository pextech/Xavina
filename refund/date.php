...
include("../config/index.php");
  $date=date("Y/m/d");
  $stmt = $DB_con->prepare("SELECT * FROM order_status WHERE stock_status='out'   AND done_date  BETWEEN '" . $from_date . "' AND  '" . $to_date . "' GROUP BY id DESC");
  $stmt->execute();
  ...
  if($stmt->rowCount() > 0)
  {
    while($row=$stmt->fetch(PDO::FETCH_ASSOC))
    {
      ...
      $re= mysqli_query($conn,"SELECT count(*) as ptotal FROM order_status WHERE order_id='$order_id'  AND done_date  BETWEEN '" . $from_date . "' AND  '" . $to_date . "' ");
       $rep = mysqli_fetch_array($re);
       $berqefa= $rep['ptotal'];

       $qtye= mysqli_query($conn,"SELECT sum(qty) FROM order_status WHERE order_id='$order_id'   AND done_date  BETWEEN '" . $from_date . "' AND  '" . $to_date . "' ");
       $qtyq = mysqli_fetch_array($qtye);
       $qtyqf= $qtyq['sum(qty)'];

       $ref= mysqli_query($conn,"SELECT sum(total) FROM order_status WHERE order_id='$order_id'  AND done_date  BETWEEN '" . $from_date . "' AND  '" . $to_date . "' ");
