...
$_SESSION['user_id']=$id;
  ...
  $sale="ordered";
 $re= mysqli_query($conn,"SELECT sum(total) FROM order_status WHERE done='$emp' AND sale_status='$sale'  AND done_date=CURDATE()");
$rep = mysqli_fetch_array($re);
 $berqef = $rep['sum(total)'];

  $salep="pending";
 $reo= mysqli_query($conn,"SELECT sum(total)  FROM order_status WHERE done='$emp' AND sale_status='$salep' AND done_date=CURDATE()");
$repo = mysqli_fetch_array($reo);
$repot = $repo['sum(total)'];
...
curl_close($ch);
	...
	$msg= "Umukozi Witwa  ".$user['fullname']."Asohotse Muri System yakoreye Amafaranga ".number_format($berqef)."Amafaranga Atishyuwe (ideni)".number_format($repot)."  Murakoze";
