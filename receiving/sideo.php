...
$invoice=$_SESSION['invoice'];
  $stmt = $DB_con->prepare("SELECT * FROM order_status WHERE stock_status='in' AND sale_status='pending' AND order_id='$invoice' ORDER BY id DESC");
  $stmt->execute();
  ...
  if($stmt->rowCount() > 0)
  {
    while($row=$stmt->fetch(PDO::FETCH_ASSOC))
    {
      extract($row);
          ...
          ?>
          <form name="refreshd" action="../dist/includes/mpages/refreshin.php" method="POST">
          <tr>
