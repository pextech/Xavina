...
$jax = mysqli_query($conn,"SELECT * FROM `order_status` WHERE order_id='$invoice' AND sale_status='$statuso'");
...
while($jqry = mysqli_fetch_array($jax)) {
	...
	$tup=$totale*$real;
    //var_dump($itenam);
	$sql = "UPDATE items SET quantity='$real',total='$tup' WHERE item_name='$itenam'";
...
//here I'm Going to update the items which is pending on the first invoice created Byimenyere

$upd=mysqli_query($conn,"UPDATE order_status SET sale_status='$status' WHERE order_id='$invoice' AND sale_status='$statuso'");
