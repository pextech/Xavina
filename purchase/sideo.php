...
$invoice=$_SESSION['invoice'];
  $stmt = $DB_con->prepare("SELECT * FROM purchase WHERE stock_status='out' AND sale_status='pending' AND order_id='$invoice' ORDER BY id DESC");
  $stmt->execute();
  ...
  if($stmt->rowCount() > 0)
  {
    while($row=$stmt->fetch(PDO::FETCH_ASSOC))
    {
      extract($row);
          ...
          ?>
          <form name="refreshd" action="../dist/includes/mpages/refresh2.php" method="POST">
          <tr>
