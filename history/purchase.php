            ...
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    ...
                    <ul class="breadcrumb">
                     ...
                     <br>
                    <div class="col-md-12">
      <button class="btn btn-sm btn-success" ><a href="../invoice/purchaseex.php" target="_blank"><font color="white"><b> Export Excel</b> </font></a></button>

                       <button class="btn btn-sm btn-success" ><a href="#" data-toggle="modal" data-target="#Purchase "><font color="white"><b> Today Order </b></font></a></button>
   </div>
     ...
     <!-- Multi Column -->
        ...
        <thead>
          <tr>
            ...
            <th ><font color="black">Exp.d</font></th>
            <th ><font color="black">SDC_N</font></th>
...
//$invoice=$_SESSION['invoice'];
  $stmt = $DB_con->prepare("SELECT * FROM purchase WHERE stock_status='out'  AND order_id='$invoiceunp' ORDER BY id DESC");
  $stmt->execute();
  ...
  if($stmt->rowCount() > 0)
  {
    while($row=$stmt->fetch(PDO::FETCH_ASSOC))
    {
      extract($row);
          ...
          ?>
          <form name="refreshd" action="../dist/includes/mpages/refreshinp.php" method="POST">
            <tr>
          ...
          <td><a class="btn btn-danger"  href="?delete_id=<?= $id ?>" title="click for delete" onclick="return confirm('Are you sure to remove <?php echo $item; ?>?')"><div class="icon-circle bg-red"><i class="zmdi zmdi-delete"></i></div></a></td>
           ...
           <td>
            ...
            <td>
              <input type="date" style="width: 60px;" class="form-control" name="date[]">
              
            </td>
            <td>
              <input type="text" class="form-control"  name="sdc[]">
                    ...
                    <div class="card">
                        <div class="header">
                          <center>  ---------------------------------------------------</center><br>
                            ...
                            $check = "SELECT customer FROM purchase where order_id='$invoiceunp'";
...
<button class="btn btn-sm btn-success" ><a href="#" data-toggle="modal" data-target="#customer"> New Customers</a></button>
  ...
  <div  role="alert" >
      ...
      $ber ="select sum(qty) from purchase where stock_status='out'  AND order_id='$invoiceunp'";
                 $berq = mysqli_query($conn,$ber) or die(mysqli_error($conn));
                 $berqe=mysqli_fetch_array($berq);
                 $berqef = $berqe['sum(qty)'];
         
         $ber ="select count(id) from purchase where stock_status='out'  AND order_id='$invoiceunp'";
      ...
      $ber ="select sum(total) from purchase where stock_status='out'  AND order_id='$invoiceunp'";
                 $berq = mysqli_query($conn,$ber) or die(mysqli_error($conn));
                 $berqe=mysqli_fetch_array($berq);
...
<br><br><br>
        <button class="btn btn-sm btn-danger" ><a  href="?delete_allid=<?= $invoiceunp ?>" title="click for delete" onclick="return confirm('Are you sure to Cancel <?php echo $invoice; ?>?')"><font color="white"><b> Cancel</b> </font></a></button>
   </form>
     <button class="btn btn-sm btn-info" ><a href="../invoice/purchase.php" target="_blank"><font color="white"><b> Print</b> </font></a></button>
